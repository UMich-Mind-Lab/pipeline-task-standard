...
if [ $failMode -eq 1 ]; then
	...
	for reg in data/${task}/preproc/acq-${acq}/sub-${sub}/ses-${ses}/run-${run}/QC/checkreg.done; do
	...
	for bold in data/${task}/preproc/acq-${acq}/sub-${sub}/ses-${ses}/run-${run}/*_utboldref.nii.gz; do
