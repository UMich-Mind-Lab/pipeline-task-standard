...
% ------------------------
% Default values for outliers %%these are the values that will be used for
% mtwins processing!!
% ------------------------
z_thresh = 99;             %global signal threshold %OVERRIDDEN BY global_threshold from config file
mvmt_thresh = 2.0;          %absolute subject motion threshold  %OVERRIDDEN by motion_threshold from config file
rotat_thresh = 0.034906585;         %absolute subject rotation threshold
mvmt_diff_thresh = 2.0;     %scan-to-scan subject motion threshold %OVERRIDDEN by motion_threshold from config file
rotat_diff_thresh = 0.034906585;    %scan-to-scan subject rotation threshold
...
if ~isempty(sess_file) % read config
    ...
    for i = 1:num_sess
        switch spm_ver
            ...
            case {12,8} %Changed from {5,8} to test for our new SPM version
